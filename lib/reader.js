...
function createReader(dbDir) {
  let throughStream = _through2.default.obj(function (fileItem, enc, next) {
    ...
    _fs2.default.readFile(fileItem.path, 'utf8', (err, data) => {
      ...
      let lines = data.trim().split('\n').map(v => JSON.parse(v)).map(v => _extends({}, v, { t: (0, _moment2.default)(v.t, [_moment2.default.ISO_8601]).toDate() }));

      lines.sort((r1, r2) => {
        if (r1.getTime() === r2.getTime()) return 0;else if (r1 > r2) return -1;else return 1;
      });
